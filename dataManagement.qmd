...
## Import Multiple Data Files {#sec-importMultipleDataFiles}
...
# Use Lab Functions {#sec-labFunctions}
...
# Load/Install Packages {#sec-loadInstallPackages}
...
# Render an `R` Markdown (`.Rmd`) File {#sec-renderRmd}
...
head(mydata$parch %in% anotherVector)
```
...
head(mydata$parch %ni% anotherVector)
```
...
head(!is.na(mydata$prediction) & mydata$parch >= 1)
```
...
head(mydata$age[which(mydata$survived == 1)])
```
...
head(mydata[which(mydata$survived == 1), "age"])
```
...
mydata3 <- data.frame(
  ID = c(1:5, 1047:1051),
  dog = sample(0:1, 10, replace = TRUE)
)
...
Unlike an [inner join](#sec-innerJoin), a left semi join will never duplicate rows of $x$, and it includes columns from only $x$ (not from $y$). 
...
## Merging Three or More Objects

```{r}
dfsToMerge <- list(mydata, mydata2, mydata3)

mergedData <- purrr::reduce(
  dfsToMerge,
  left_join, # could change to full_join (or whichever join is preferred)
  by = "ID")
