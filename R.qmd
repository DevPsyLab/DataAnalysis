...
1. In the main program files directory, install one of the following graphical use interfaces (GUIs) to `R` (you may have to right click and "Run As Administrator"):
    - `RStudio` Desktop (<https://www.rstudio.com/products/rstudio/download/>)
    - `Positron` (<https://positron.posit.co>)
1. Set the executables for `R` and `RStudio`/`Positron` to always run with administrator permissions.
...
To do this, perform the following steps:
    ...
    - Download the lab's `Rprofile.site` file located in this repository at the following location (<https://research-git.uiowa.edu/PetersenLab/R-InitialSetup/-/blob/master/R%20Setup%20Files/Rprofile.site>), and paste it into the `R` installation directory
        - PC: `C:/R/R-[InsertVersionNumber]/etc/Rprofile.site`
        - Mac: `/Library/Frameworks/R.framework/Versions/[InsertVersionNumber]/Resources/etc/Rprofile.site`
...
1. For [reproducibility purposes](#sec-dontSaveWorkspace), prevent `R`/`RStudio`/`Positron` from saving your workspaces automatically using the following steps:
...
1. Request an [API token](https://redcap.icts.uiowa.edu/redcap/api/help/?content=tokens) for the following REDCap project(s); note: please check with Dr. Petersen before requesting an API token.
...
1. Add the SRS Data Processing repo from the lab drive to your `GitHub Desktop` App (`//lc-rs-store24.hpc.uiowa.edu/lss_itpetersen/Lab/Studies/School Readiness Study/Data Processing`)
    - For instructions, see the section on "[How to add a pre-existing repo from the lab drive (LSS/RDSS/network share) to your computer](git.qmd#sec-preExistingRepoLabDrive)" here: <https://research-git.uiowa.edu/petersenlab/R-InitialSetup/blob/master/GitLab/GitLab%20Instructions.md>
1. Open `RStudio`/`Positron` by using "Run as Administrator" (always open it as an administrator so it has write access to the program files directory);
...
1. For antialiased plots in `RStudio`/`Positron`, change the Graphics backend to `Cairo`:
...
These environments are analogous to [containers](hpc.qmd#sec-containers) in that they contain all of the necessary scripts and packages to recreate a given project.
...
1. Start R
1. Load the `renv` package:

    ```r
    library("renv")
    ```
    
1. When prompted, initialize a `renv` project:

    ```r
    renv::init(project = "/path/to/project/directory/")
    ```
...
# 5. Generate the PDF version of the manual and place it in the `/man/figures/` folder of the package:
#devtools::build_manual(path = "./man/figures/manual.pdf")
```

5. Generate the PDF version of the manual and place it in the `/man/figures/` folder of the package using the following code in the terminal (similar to `devtools::build_manual(path = "./man/figures/")`, but can name the file `manual.pdf`):

    ```bash
    cd ./petersenlab
    R.exe CMD Rd2pdf . --output=man/figures/manual.pdf --force --no-preview --no-clean
    ```

6. Either manually or using the terminal, delete the created folder whose name begins with ".Rd2pdf..."—e.g.:

    ```bash
    rm .Rd2pdf39632 -Recurse -Force
    ```

```
# 7. Then, build the package: Ctrl-Shift-B
devtools::build()

# 8. Then, check the package: Ctrl-Shift-E
devtools::check()
```

9. Then, run [`R CMD check`](#sec-rCmdCheck)

10. Then, install the package:
...
devtools::build() # or: devtools::build(pkg = "E:/Documents/GitHub/petersenlab")
```

1. Open terminal in RStudio/Positron
...
1. Run `R CMD check --as-cran`
    ...
    Then, run `R.exe CMD check --as-cran` followed by the name of your package tarball (making sure to update the package version in the filename). For example:

    ```bash
    R.exe CMD check --as-cran petersenlab_1.0.0.tar.gz
    ```
...
## Managing packages for a project using `packrat` {#sec-packrat}
...
This is relevant if building a `packrat` on [Argon/HPC](hpc.qmd).
