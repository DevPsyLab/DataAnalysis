...
ANALYSIS:
    ...
    ROTATION = GEOMIN;
    PARALLEL = 50; ! the number of random data sets that is drawn for parallel test

PLOT:
    TYPE = PLOT2; ! sample eigenvalues, the parallel analysis eigenvalues, and the parallel analysis eigenvalues for the 95th percentile
```
...
## Indirect Effects (Mediation) {#sec-indirectEffects}
...
The example below is adapted from Hoffman's [Example 5b](https://www.lesahoffman.com/PSQF7375_AdvLong/index.html) (archived at <https://perma.cc/N2AR-52S5>).
For other examples, see [here](https://research-git.uiowa.edu/PetersenLab/Mplus-Multilevel-SEM/-/tree/main/Analyses/Mplus)[^1].
...
VARIABLE:
    ...
    LAGGED  = A(1) B(1);     ! Create Mplus lag-1 variables (i.e., t-1)
...
MODEL: ! A = construct 1; B = construct 2
  %WITHIN%
    ...
    A^ ON A^1 (aAR);        ! (optional) L1 WP fixed effect of autoregressive stability of ^residual A
    B^ ON B^1 (bAR);        ! (optional) L1 WP fixed effect of autoregressive stability of ^residual B
  ...
  %BETWEEN%
    ...
    Aquad@0;                           ! No quadratic change variance
...
- Allow the cross-lagged effects to vary as a function of time (add time as a moderator of those effects); see Hamaker's note [here](https://www.statmodel.com/discussion/messages/11/25297.html?1579816772) (archived at <https://perma.cc/VRA2-FTGL>)
