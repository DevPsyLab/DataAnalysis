...
#### Proportion Mediated (*P*<sub>*M*</sub>)
...
#### Proportion of Variance in Y Explained by the Indirect Effect (*R*<sup>2</sup><sub>mediated</sub>)
...
```{r}
rSquaredXY <- as.numeric(cor.test(
  ~ X + Y,
  data = mydata
)$estimate) ^ 2

rSquaredMY <- as.numeric(cor.test(
  ~ M + Y,
  data = mydata
)$estimate) ^ 2
...
RsquaredMed <- rSquaredMY - (RsquaredYmx - rSquaredXY)
RsquaredMed
```

#### Ratio of the Indirect Effect Relative to Its Maximum Possible Value in the Data ($\kappa^2$)
...
#### The Variance in Y Accounted for Jointly by M and X (upsilon; $v$)

$$
v = \beta^2_a \cdot \beta^2_b
$$
...
```{r}
aPath <- mediationFit_estimates %>% 
  filter(label == "a") %>% 
  select(std.all) %>% 
  as.numeric

bPath <- mediationFit_estimates %>% 
  filter(label == "b") %>% 
  select(std.all) %>% 
  as.numeric

aPathSquared <- aPath ^ 2
bPathSquared <- aPath ^ 2

upsilon <- aPathSquared * bPathSquared
upsilon
```
