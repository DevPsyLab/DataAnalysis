...
plot_data$slope_se <- sqrt((b1_se)^2 + (plot_data$Murder_centered^2 * b3_se)^2 + 2 * plot_data$Murder_centered * b1_se * b3_se)
...
plot_data$significant_slope[which(plot_data$Murder_centered < -4.2 | plot_data$Murder_centered > -0.2)] <-TRUE # specify significant range
...
plot_data$significantRegionNumber[which(plot_data$Murder_centered > -0.2)] <- 2 # specify significant range 2
...
ggplot(plot_data, aes(x = Murder, y = predicted_slopes)) +
  ...
  geom_vline(xintercept = c(4.051215, 8.049275), linetype = 2, color = "#00BFC4") + # update based on numbers above
  labs(
