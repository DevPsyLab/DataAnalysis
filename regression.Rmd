...
```{r}
pairwiseData <- mydata[,c("bpi_antisocialT2Sum","bpi_antisocialT1Sum","bpi_anxiousDepressedSum")]
varMeans <- colMeans(pairwiseData, na.rm = TRUE)
varCovariances <- cov(pairwiseData, use = "pairwise")
...
pairwiseRegression_fit <- lavaan(
  ...
  sample.nobs = sum(complete.cases(pairwiseData))
)
