...
N        <- 1200  # independent sample size at each timepoint
...
# true theta distributions -- shifted upward across timepoints
theta1_true <- rnorm(N, -0.5, 1)
theta2_true <- rnorm(N,  0.0, 1)
theta3_true <- rnorm(N,  0.5, 1)

dat1 <- gen_data(theta1_true, item_names_T1, list(U1, C12))
dat2 <- gen_data(theta2_true, item_names_T2, list(C12, U2, C23))
dat3 <- gen_data(theta3_true, item_names_T3, list(C23, U3))

dat1 <- as.data.frame(dat1); dat2 <- as.data.frame(dat2); dat3 <- as.data.frame(dat3)

# prepare data for concurrent calibration
all_items <- union(union(colnames(dat1), colnames(dat2)), colnames(dat3))

make_wide <- function(dat, all_items) {
  out <- matrix(NA_integer_, nrow(dat), length(all_items), dimnames = list(NULL, all_items))
  out[, colnames(dat)] <- as.matrix(dat)
  as.data.frame(out)
}

dat_concurrent <- rbind(
  make_wide(dat1, all_items),
  make_wide(dat2, all_items),
  make_wide(dat3, all_items))
...
fit1 <- mirt::mirt(dat1, 1, itemtype = "graded", SE = TRUE)
fit2 <- mirt::mirt(dat2, 1, itemtype = "graded", SE = TRUE)
fit3 <- mirt::mirt(dat3, 1, itemtype = "graded", SE = TRUE)
```
...
rownames(cfs1) <- colnames(dat1)
rownames(cfs2) <- colnames(dat2)
rownames(cfs3) <- colnames(dat3)
...
coef(fit_concurrent, simplify = TRUE)  # inspect per-group means/variances

theta_concurrent <- fscores(fit_concurrent, method = "EAP")
th1_conc <- theta_concurrent[group == "T1"]
th2_conc <- theta_concurrent[group == "T2"]
th3_conc <- theta_concurrent[group == "T3"]
```
