...
# Markdown {#sec-rMarkdown}

## Overview {#sec-overview}

This is an [`Quarto`](https://quarto.org) document that is written using Markdown.
...
For more details on using [`Quarto`](https://quarto.org), see <https://quarto.org>.
...
Curly quotes are often inserted when copying and pasting from other sources (e.g., Word documents, websites), so make sure to convert them to straight quotes.
...
- As noted in the [links](#sec-links) section, link to the heading label, and do not use the website-derived link for a given heading because it is not a stable link; the heading path can change if other headings are added.
- Use full (absolute) filepaths (e.g., `\\lc-rs-store24.hpc.uiowa.edu\lss_itpetersen\Lab\Studies\`) rather than paths relative to a particular mapped drive letter (which can differ from computer to computer; (i.e., `Z:\Lab\Studies\`)).

## Updating {#sec-updateWiki}
...
## Microsoft Visual Studio Code {#sec-vsCode}
...
### Creating a Code Chunk {#sec-createCodeChunk}

Add a new chunk by clicking the *Insert Chunk* button on the toolbar or by pressing *Ctrl+Alt+I*.

### Running a Code Chunk {#sec-runCodeChunk}
...
### Creating the HTML Output {#sec-htmlOutput}
...
# Computational Notebook {#sec-computationalNotebook}

## General Info {#sec-general}
...
To use the lab templates for the "Prepare Data" and "Computational Notebook" scripts, first install the [`petersenlab` package](R.qmd#sec-petersenlab).
...
# `rmarkdown`/`Quarto` Website using `GitHub Actions` {#sec-website}

`Quarto` markdown files use the `.qmd` extension, whereas `rmarkdown` files use the `.rmd` extension.

## Steps to Create {#sec-websiteSteps}
...
The lab template for an APA-style Quarto open access manuscript is located at the following location: <https://github.com/DevPsyLab/QuartoManuscript>.
...
- <https://github.com/jgeller112/Beta_regression_tutorial>
    - <https://jgeller112.github.io/beta_regression_tutorial/>
- <https://github.com/mine-cetinkaya-rundel/indo-rct>
    - <https://mine-cetinkaya-rundel.github.io/indo-rct/>
- <https://github.com/alex-koiter/riparian-grazing-manuscript>
    - <https://alexkoiter.ca/riparian-grazing-manuscript/>
...
## Troubleshooting {#sec-troubleshootingMarkdown}
...
### Subsubheading {#sec-subsubheading}

#### Subsubsubheading {#sec-subsubsubheading}
...
We use asterisks for [adding emphasis to text](#sec-emphasis).
...
# Code/Syntax Highlighting {#sec-codeHighlighting}
...
To link to a heading that is in the same wiki, make sure the heading has a label (e.g., `{#sec-label}`), as shown [here](#sec-headings).
Then, link to the heading with the following syntax: `[text to show](#sec-label)`.
For example, to link to the links section, you would specify: `[text to show](#sec-links)`.
If the heading is on a different page of the same wiki, specify the html file before the label, e.g.: `[text to show](dataManagement.qmd#sec-saveData)`.
...
[I am a link to another file in this project](pca.html#sec-overview)
...
## Subscript/Superscript {#sec-subSuperscript}

### Markdown {#sec-markdownSubSuperscript}
...
## Definition List {#sec-definitions}
...
To cross-reference a table, add a caption below the table, then include a `#tbl-` label in braces at the end of the caption.
For example:
...
# Adding a Drop Down List {#sec-dropDownList}
...
# Embedding a Picture {#sec-picture}
...
Follow the instructions [here](git.qmd#sec-gitLfs) to do so.
...
# Embedding a PDF in a Markdown document {#sec-pdf}
...
# Embedding a HTML file within a Markdown document {#sec-htmlEmbed}
...
# Converting Word Documents to Markdown {#sec-Word}

The process of converting word documents to Markdown is aided by having knowledge of the [best practices](#sec-bestPractices) steps.
...
1. Follow the [best practices](#sec-bestPractices) to convert the document to Markdown format
   -  For example, in the manuals for the School Readiness Study, each [new header](#sec-headings) should denote a new task
...
# Video formatting for Wiki {#sec-videoFormatting}
...
## Lowering File Size {#sec-lowerFileSize}
