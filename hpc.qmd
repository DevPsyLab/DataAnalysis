...
- `module avail` lists the modules available to use
    - Narrow the search by specifying a search term (e.g., `module avail R` to show `R` modules)
...
- `sudo insertCommandName` runs the provided command with administrative privileges
    - `sudo apt update && sudo apt install -y dos2unix` installs the `dos2unix` package
...
However, there is a more recent stack (the 2022.2 stack) that has more current versions of the software in it (see the [software list](https://uiowa.atlassian.net/wiki/spaces/hpcdocs/pages/76513504/2022.2+stack#Software-List)).
