...
1. If you are on Windows, install Windows Subsystem for Linux (WSL)
    - In PowerShell (Admin):
    ```bash
    wsl --install
    ```
...
1. Create your `R` Container Definition File
    - You can either download a container definition files from [here](https://research-git.uiowa.edu/PetersenLab/HPC/-/tree/master/Argon/Containers/DefinitionFiles), or you can create it manually (as described below).
    - Inside the Ubuntu terminal, run:
    ```bash
    nano r_argon.def
    ```
