...
- `qsub jobFile.job` submits the specified job file to the [queue](#sec-queues)
...
See below for instructions on running `R` scripts on Argon:
...
However, we now run the most recent version of `R` (and packages) using a [container](#sec-containers).

### Compile Another Version of `R`
...
Link to UI Biostat documentation: <https://iowabiostat.github.io/hpc/10.html> (archived at <https://perma.cc/2BFA-JJK5>)
...
1. Update Ubuntu
    ...
    1. If you wan to see what version of Ubuntu you're running, enter `lsb_release -a`
1. Install Apptainer
    ...
    1. If Apptainer was already installed (i.e., you did not just install it) and you want to upgrade the Apptainer installation, run the following code to upgrade Apptainer:
        ...
        - If you had previously installed the old Apptainer version from source, you may have to remove it:

            <details>
            <summary>Remove Source Installation</summary>

            ```bash
            sudo rm /usr/local/bin/apptainer
            ```
...
However, there are a handful of necessary components for a definition file, which you can read about [here](https://apptainer.org/docs/user/latest/definition_files.html#definition-files) (archived at <https://perma.cc/D8YM-AHF8>).
...
1. If installing `MATLAB`:
    1. Copy the dependencies file into your `containers` folder (get the dependencies path for the latest released version from here: <https://github.com/mathworks-ref-arch/container-images/tree/main/matlab-deps>; make sure to specify the correct architecture—i.e., AMD or ARM; to find the architecture, enter `uname -m` where `x86_64` is AMD and `aarch64` is ARM):

        ```bash
        wget -q -O base-dependencies.txt https://raw.githubusercontent.com/mathworks-ref-arch/container-images/refs/heads/main/matlab-deps/r2026b/ubuntu24.04/base-dependencies-amd64.txt
        ```

1. Create or download the definition file
    1. To download an existing definition file (see [GitLab](https://research-git.uiowa.edu/PetersenLab/HPC/-/tree/master/Argon/Containers/DefinitionFiles) for options—make sure to update the version numbers for MATLAB, Python, and the Ubuntu OS as necessary):
        1. Copy the definition file into your containers folder:

            ```bash
            cp /mnt/path/to/def/file.def ~/containers
            # For example:
            # cp /mnt/c/Users/USERNAME/Downloads/file.def ~/containers
            # cp /mnt/e/Documents/GitHub/HPC/Argon/Containers/DefinitionFiles/all_softwares.def ~/containers
            ```
        ...
        1. Add remaining parameters
            ...
            <summary>All Software</summary>
              ...
              EOF
                  ...
                  R -e "remotes::install_github('Jinsong-Chen/LAWBL')"
...
1. Copy the container to your computer:
    ...
    ```bash
    cp ~/containers/argon_container.sif "/mnt/c/Users/hawkid/path/to/folder/"
    # For example:
    # cp ~/containers/argon_container.sif "/mnt/e/Documents/OneDrive - University of Iowa/Research/Containers/"
    ```
...
1. If you installed `CmdStan`, copy the executable files to a writable location:
    ...
    # for example:
    # apptainer exec /Users/itpetersen/Documents/Containers/argon_container.sif \
    #   cp -r /opt/cmdstan/. $HOME/cmdstan_cache
    ```
...
Basic Job Submission: <https://uiowa.atlassian.net/wiki/spaces/hpcdocs/pages/76513450/Basic+Job+Submission> (archived at <https://perma.cc/2SS2-LEJR>)

Advanced Job Submission: <https://uiowa.atlassian.net/wiki/spaces/hpcdocs/pages/76513452/Advanced+Job+Submission> (archived at <https://perma.cc/8H6G-2M2F>)
...
- `qstat` shows the status of available [queues](#sec-queues) and jobs
    - `qstat -u hawkid` shows jobs submitted by the specified HawkID
    - `qstat -g c -q QUEUE_NAME` shows the status of the specified [queue](#sec-queues)
    - `qstat -j JOB_ID` shows detailed status info for the specified job (if still running)
    ...
    - Status Abbreviations:
        ...
        - `R` - restarted
        - `s`  `S` - job (s) or [queue](#sec-queues) (S) suspended
        - `T` - [queue](#sec-queues) threshold exceeded (suspended)
        - `w` - waiting in [queue](#sec-queues)
...
1. Add `qsub` options (optional)
    ...
    A list of `qsub` options is provided in @sec-qsubOptions.
...
| Option  | Description |
| ------- | ----------------------------------------  |
| `-cwd`  | execute the job from the current working directory; if not specified, the job will be run from your home directory. |
...
| `-l mf=[nG\|nM]` | number (`n`) of gigabytes (`G`) or megabytes (`M`) of free memory needed at startup (e.g., `256G`) |
| `-l h_vmem=[nG\|nM]` | number (`n`) of gigabytes (`G`) or megabytes (`M`) of virtual memory needed across entire job (e.g., `256G`) |
...
| `-pe smp [cores]` | number of slots (CPU cores) to use[^1]  |
| `OMP_NUM_THREADS=[n]` | number of OpenMP threads to use per MPI process (default = 1) |
| `-q [UI\|UI-HM\|UI-MPI\|UI-GPU\|UI-DEVELOP\|all.q]` | which job [queue](#sec-queues) to use  |
...
`-pe smp 4`: specify number of slots (CPU cores) to be used (`smp` = shared memory parallel environment); increasing the number of slots increases the memory available for the job.
...
You can find the specific configurations for each node type on the wiki: <https://uiowa.atlassian.net/wiki/spaces/hpcdocs/pages/76513468/Queues+and+Policies> (archived at <https://perma.cc/UUR7-XLBZ>).
For instance, if you are using the UI [queue](#sec-queues) that has 512GB of memory spread across 128 cores, each core comes with 4GB of memory (i.e., $512 / 128 = 4$).
...
apptainer exec \
    ...
    --cleanenv \
    /Users/itpetersen/Documents/Containers/argon_container.sif \
...
`UI-HM`: for jobs requiring high memory

`UI-GPU`: for jobs that can use a GPU accelerator

`UI-GPU-HM`: for jobs that can use a GPU accelerator and that require high memory

`UI-DEVELOP`: recommended for test runs and debugging

`UI-MPI`: for jobs that can use multiple nodes simultaneously

`all.q`: the lowest priority queue, so jobs running on it will "give up" resources if needed for jobs in higher-priority queues

The `UI` and `UI-HM` queues are the most used by DevPsy.
