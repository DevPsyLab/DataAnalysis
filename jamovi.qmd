...
# Overview of `jamovi` {#sec-jamovi}

[`jamovi`](https://www.jamovi.org) is similar to [`SPSS`](spss.qmd) in that it has a graphical user interface that allows point-and-click for analysis.
Unlike [`SPSS`](spss.qmd), however, [`jamovi`](https://www.jamovi.org) is free.
[`jamovi`](https://www.jamovi.org) can also provide the [`R`](R.qmd) code that used to perform the analysis.
