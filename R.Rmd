...
- UCLA Stats Website: https://stats.oarc.ucla.edu/other/dae/
...
1. Set up git, GitLab, and the GitHub Desktop App in the main program files directory; may have to right click and "Run As Administrator"; For instructions setting up and using GitLab, see here: https://devpsylab.github.io/DataAnalysis/git.html#21_To_begin
...
To do this, perform the following steps:
    ...
    Then, add/edit `HOME` as the "variable name" with the intended location as the "variable value" (e.g., `C:/Users/[user]/Documents`, where you replace "user" with your HawkID).
        - You may also solve this issue by placing the following command in the `Rprofile_site` from the previous step
...
The solution to this problem [can be found at this link](https://stackoverflow.com/questions/28432607/pandoc-version-1-12-3-or-higher-is-required-and-was-not-found-r-shiny)
...
Occasionally, it can be helpful to send a slack using R.
For example, if a script does not run, a slack can be sent to inform the appropriate team members.[These instructions](https://www.infoworld.com/article/3402657/how-to-slack-from-r.html) can largely be followed to set up R. However, there are some differences:
...
It is also possible to slack specific users with instructions found [at this link](https://stackoverflow.com/questions/32419756/how-do-you-tag-people-with-a-slack-bot)

# Replacing `//n` with a space

Many notes in the school readiness study that are exported from REDCap come with spaces denotes as `//n`.
