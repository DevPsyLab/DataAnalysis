...
In the terminal, change the directory to where the `.tar.gz` file is located (commonly up one directory):

```bash
cd ..
```

In the terminal, Run `R CMD check` (making sure to update the package version in the filename):
...
In the R Console, build the source package in `R` (or using the instructions described [above](#rCmdCheck)):
