...
1. Open the syntax file and make sure lab path is saved as the absolute file path in the (`/FILE=`) field (**not** a mapped network drive like `R:`)
   * `\\iowa.uiowa.edu\Shared\ResearchData\rdss_itpetersen\Lab\Members\HAWKID\Thesis\Data\srs_idWave.csv`
...
```
GET DATA  /TYPE=TXT
  /FILE="\\iowa.uiowa.edu\Shared\ResearchData\rdss_itpetersen\Lab\Members\HAWKID\Thesis\Data\srs_idWave.csv"
  /ENCODING='UTF8'
  /DELIMITERS=","
  /QUALIFIER='"'
...
SAVE OUTFILE='\\iowa.uiowa.edu\Shared\ResearchData\rdss_itpetersen\Lab\Members\HAWKID\Thesis\Data\srs_idWave.sav'
  /COMPRESSED.
```
...
First, download and run the [`PROCESS` macro](http://www.processmacro.org/download.html).
...
1. Run moderated multiple regression model
   ...
   /NOORIGIN 
   /DEPENDENT outcome
   /METHOD=ENTER predictor_MC moderator_MC predictorXmoderator.
   ```
