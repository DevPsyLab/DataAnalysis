...
- `~~` indicates covariance (i.e., unstandardized correlation; it is a correlation, i.e., standardized covariance, if the variables—or the coefficient—are standardized)
    ...
    - `~ 1` indicates intercept
        ...
        - To center a variable, you can set its intercept to 0:
            - `variable ~~ 0*1`
- `LABEL*variable` provides a user-specified label to the parameter
    - any parameters that are given the same label will be fixed to be the same
    - any parameters that are given different labels will be allowed to differ
...
latent4 ~ NA*x1 + x2 + x3 # free factor loading of first indicator (it is set to 1 by default)
```
...
latent2 <~ label1*x1 + label2*x2 + label3*x3 # parameter labels
latent3 <~ label1*x1 + label1*x2 # constrain two parameters (factor loadings) to be the same using a common parameter label
latent4 <~ 1*x4 # fix factor loading to 1
```
...
y4 ~ 1*x1 # fix regression path to 1
```
...
x1 ~~ 0*x2 # fix covariance to 0 (uncorrelated)
```
...
Parameter labels for multiple groups (have as many labels as there are groups; making sure to specify the grouping variable in the `group` argument):
...
# Latent Growth Curve Model (LGCM) {#sec-lgcm}
...
- growth curves using latent indicators (rather than observed indicators): <https://tdjorgensen.github.io/SEM-in-Ed-compendium/ch27.html#latent-indicators-of-growth-factors> (archived at <https://perma.cc/7HMC-AGNP>)
- growth curves using discrete indicators (rather than continuous indicators): <https://tdjorgensen.github.io/SEM-in-Ed-compendium/ch27.html#discrete-indicators> (archived at <https://perma.cc/LT84-82EY>)
- parallel growth curves: <https://tdjorgensen.github.io/SEM-in-Ed-compendium/ch27.html#parallel-growth-curves> (archived at <https://perma.cc/A6WA-627J>)
...
lgcm1_fit <- growth(
  ...
  int_lv_free = TRUE,
  fixed_x = FALSE,
  em_h1_args = list(max_iter = 100000)
)
```
...
lavaangui::plot_lavaan(logGCM1_fit)
```
...
```{r}
logGCM1_intercept <- coef(logGCM1_fit)["intercept~1"]
...
## Second-Order Latent Growth Curve Model {#sec-secondOrderLGCM}

An example of a second-order latent growth curve model is [here](#sec-secondOrderLGCM).

# Latent Change Score Model (LCSM) {#sec-lcsm}
...
# Cross-Lagged Panel Model (CLPM) {#sec-clpm}
...
# Random Intercept Cross-Lagged Panel Model (RI-CLPM) {#sec-riclpm}
...
# Autoregressive Latent Trajectory (ALT) Model {#sec-alt}

As noted by Curran and Hancock (2021) and in their [corresponding episode on Quantitude](https://quantitudepod.org/s7e20-co-development/), in an ALT model, the time-linked relations affect the underlying growth trajectories and vice versa, thus preventing the pure isolation of within-person and between-person effects.
Thus, researchers advocate for the use of the [latent curve model with structured residuals](#sec-lcm-sr) (LCM-SR) to separate within-person and between-person effects.
...
lavaangui::plot_lavaan(alt_fit)
```

# Latent Curve Model with Structured Residuals (LCM-SR) {#sec-lcm-sr}

A latent curve model with structured residuals (LCM-SR) is an extension of the [autoregressive latent trajectory (ALT) model](#sec-alt).
A LCM-SR model is also called an autoregressive latent trajectory model with structured residuals (ALT-SR).
The LCM-SR separates stable between-person differences in developmental trajectories from within-person deviations around those trajectories.
The latent growth factors (e.g., intercept and slope) capture between-person differences, whereas the autoregressive and cross-lagged relations among the structured residuals capture within-person dynamics.
...
mediationFit <- sem(
  ...
  missing = "ML",
  estimator = "ML",
  # std_lv = TRUE, # for models with latent variables
  fixed_x = FALSE)
```
...
# Moderation/Interaction {#sec-moderation}
...
## Latent Variable Moderation/Interaction {#sec-moderationLatent}

There are several options for examining latent variable moderation/interaction:

- Latent Moderated Structural equations (LMS) approach (e.g., [`modsem` package](https://modsem.org) and [`Mplus`](mplus.qmd))
- local structural-after-measurement (SAM) (for papers, see [here](https://doi.org/10.3758/s13428-024-02532-y) and [here](https://doi.org/10.1037/met0000851))
- product indicator approach (e.g., [`semTools` package](https://www.mdpi.com/2624-8611/3/3/24))
...
round(cbind(
  configural = inspect(configuralInvarianceCorrelatedResiduals_fit, "fit.measures"), # note: fit.measures may be deprecated in future versions in favor of snake_case (fit_measures)
...
round(cbind(
  model1 = inspect(model1, "fit.measures"), # note: fit.measures may be deprecated in future versions in favor of snake_case (fit_measures)
  model2 = inspect(model2, "fit.measures"),
  model3 = inspect(model3, "fit.measures")), 3)
```
...
# Auxiliary Variables to Account for Missingness {#sec-auxiliaryVariables}
...
includingAuxVars <- cfa(
  HS.model,
  data = HolzingerSwineford1939,
  missing = "ML",
  estimator = "MLR",
  aux = c("sex","ageyr","grade") # include auxiliary variables here
)
```
...
- [`simsem` `R` package](https://cran.r-project.org/web/packages/simsem/index.html) (see vignettes [here](https://github.com/simsem/simsem/wiki/Vignette))
...
- [`powRICLPM` `R` package](https://cran.r-project.org/web/packages/powRICLPM/index.html) for [random-intercept cross-lagged panel models](#sec-riclpm) (based on [Mulder, 2023](https://doi.org/10.1080/10705511.2022.2122467); see vignettes [here](https://jeroendmulder.github.io/powRICLPM/articles/start.html))
...
lavInspect(configuralInvariance_fit, "sigma.hat") # note: sigma.hat may be deprecated in future versions in favor of snake_case (sigma_hat)
...
```{r}
lavInspect(configuralInvariance_fit, "cov.lv") # note: cov.lv may be deprecated in future versions in favor of snake_case (cov_lv)
...
```{r}
eigen(lavInspect(configuralInvariance_fit, "sigma.hat"))$values # note: sigma.hat may be deprecated in future versions in favor of snake_case (sigma_hat)
eigen(lavInspect(configuralInvariance_fit, "cov.lv"))$values # note: cov.lv may be deprecated in future versions in favor of snake_case (cov_lv)
```
