...
https://wiki.uiowa.edu/display/hpcdocs/Cluster+Systems+Documentation (archived at https://perma.cc/36D7-4ATD)

https://wiki.uiowa.edu/display/hpcdocs/Argon+Cluster (archived at https://perma.cc/72Z6-5DYG)
...
https://wiki.uiowa.edu/display/hpcdocs/Argon+Software+List (archived at https://perma.cc/33U7-VQAH)
...
https://wiki.uiowa.edu/display/hpcdocs/Basic+Job+Submission (archived at https://perma.cc/5JAS-HKS4)

https://wiki.uiowa.edu/display/hpcdocs/Advanced+Job+Submission (archived at https://perma.cc/W79J-F9RT)
...
https://wiki.uiowa.edu/display/hpcdocs/Queues+and+Policies (archived at https://perma.cc/MQ5W-TSRD)
...
https://wiki.uiowa.edu/display/hpcdocs/R+Programs+in+Batch+mode+for+HPC (archived at https://perma.cc/X5N5-TT3H)

## Find which versions of R are installed

`module spider R`

But, there may be more recent version of R installed in the "Additional Software Stacks" (https://wiki.uiowa.edu/display/hpcdocs/Argon+Software+List; archived at https://perma.cc/33U7-VQAH)

## Install R packages

https://wiki.uiowa.edu/display/hpcdocs/Adding+R+programs+to+a+personal+library (archived at https://perma.cc/7KQY-EZHQ)
...
```r
install.packages(c(
    "packrat","psych","tidyverse","data.table","nlme","lme4","mirt","TeachingDemos","Amelia","mice","miceadds","abind","future","lavaan","blavaan","Rcpp","igraph","shinystan","StanHeaders","brms","rstan","rjags","renv"),
...
1. At the R prompt, initialize the `renv` project on the local repository of R packages with: `renv::init(project = "/Users/itpetersen/Documents/Projects/SelfRegulation_IRT/")`
...
1. `renv::snapshot()` or `packrat::snapshot()`; if that command fails due to an error when fetching sources, try `packrat::snapshot(snapshot.sources = FALSE)`

### Build packrat environment on a compute node (only if necessary if package load fails due to issues building packages)

1. `qlogin` -- for more info, see here: https://wiki.uiowa.edu/display/hpcdocs/Qlogin+for+Interactive+Sessions (archived at https://perma.cc/4B47-HHYJ)
...
http://gforge.se/2015/02/how-to-go-parallel-in-r-basics-tips/#Memory_load (archived at https://perma.cc/2JRF-8Y5F)
