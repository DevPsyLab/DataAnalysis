...
1. If that does not work, you can try downloading the .tar file directly from the CRAN repository. Then copy the file into your Argon folder and type 'install.packages("/Users/path/to/directory/package_name", repos = NULL, type = "source")'. Note that you may need to download an older version of the package, such that it is compatible with the version of R you are running on Argon (which it typically not the most recent R version).
1. If steps 1 and 2 fail, email research-computing@uiowa.edu and ask them to install the packages for you.
