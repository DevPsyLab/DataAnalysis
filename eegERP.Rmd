...
1. Save a copy of all extracted PCA component files (e.g., `erp_ob_tgt_all_P3`) in the following location on the Lab Drive: `Lab/Studies/School Readiness Study/Data/LV2/PCA Components` under the appropriate task (i.e., Oddball, Fishshark, or Stop Signal)
1. Use the following R Code to combine all components into a single composites datafile.
The following example is taken from the `erpOddball.R` composite script located in `Lab/Studies/School Readiness Study/Data Processing/3. Composites`.
Comparable scripts have also been created for FishShark and Stop Signal tasks.
   ...
   This list will contain the full filepaths to each component file which are used to read the data into R.
      ...
      oddballFilePaths <- list.files(
       path = file.path(petersenLabPath, "/Lab/Studies/School Readiness Study/Data/LV2/ERP/PCA Components/Oddball"), 
       pattern = "*.txt",
       recursive = FALSE,
       full.names = TRUE)
      ```
   ...
   1. "Clean" the list of component file names by removing the file extension (in this case, `.txt`).
    ...
    1. Save composite data to desired location.
       ...
       ```
       write_csv(oddballDataClean, file = file.path(petersenLabPath, "/Lab/Studies/School Readiness Study/Data/REDCap/Composites/erpOddball.csv"))
       ```
# Visualizations in R 
