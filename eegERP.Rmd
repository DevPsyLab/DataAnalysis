...
## Moving data from LV2 Observation Room to File Storage
1. Power all the LV2 computer equipment down
1. Find the black box (the external harddrive) behind the Apple computer monitor
1. Unplug the black box
1. Plug it in to the computer you plan to perform processing onthe computer you plan to perform EEG processing on (note--will likely need large file storage on the computer)
1. Move the .mff files to the raw file folder (1a- Files for HAPPE) on the ERP processing repository stored on your large file storage drive
1. Power off your lab computer to unplug the black box and put it back
...
1. Open the `eegProccessingOddball.m` file (stored here: `/Data Processing/6. MATLAB EEG Pipeline/`) in MATLAB
...
1. Open the `eegProccessingFishshark.m` file (stored here: `/Data Processing/6. MATLAB EEG Pipeline/`) in MATLAB
1. Update all thresholds and filepaths in script file (must be done BEFORE running the script)
...
The following example is taken from the `erpOddball.R` composite script located in `[...]Data Processing/3. Composites`.
