...
1. Open the `eegProccessingOddball.m` file (stored here: `/Data Processing/6. MATLAB EEG Pipeline/`; accessible via Git [here](https://research-git.uiowa.edu/PetersenLab/SRS/SRS-DataProcessing/-/blob/master/6.%20MATLAB%20EEG%20Pipeline/eegProcessingOddball.m)) in MATLAB
...
1. Generate a .txt file listing all processed .mff files
   ...
   - The list of processed files is generated using an environment variable (`FileNames`) that the HAPPE pipeline creates that lists all files inputted to the pipeline
...
## Fish/Shark Processing Pipeline

### MATLAB Script

The following section describes the MATLAB scripts used to manage HAPPE input and output files for the Fish/Shark task data. If you are running HAPPE manually (i.e., without integrating these scripts), skip to the [Fish/Shark HAPPE inputs](#inputFishSharkHAPPE) section.

**The first two steps dsescribed in this section are the same as those described for Oddball processing.**
**Processing actions specific to FishSharks task begin in Step 3**

1. Open the `eegProccessingFishshark.m` file (stored here: `/Data Processing/6. MATLAB EEG Pipeline/`; accessible via Git [here](https://research-git.uiowa.edu/PetersenLab/SRS/SRS-DataProcessing/-/blob/master/6.%20MATLAB%20EEG%20Pipeline/eegProcessingFishshark.m)) in MATLAB
1. Update all thresholds and filepaths in script file (must be done BEFORE running the script)
...
After the HAPPE inputs are entered, no more user interaction is required for the script to do its job.
The HAPPE pipeline will run, and the remaining MATLAB code in the script file will evaluate the files outputted by HAPPE and move them to the appropriate locations based on this evaluation. 
See [Fish/Shark Post-HAPPE Steps](#FishSharkPost) for a detailed explanation of the code used to accomplish this.
...
### Fish/Shark Post-HAPPE Steps {#fishSharkPost}
...
This file corresponds to the electrode template associated with the participant age range and type of net represented in the data
   ...
   ![image](images/netTemplateFile.png)
