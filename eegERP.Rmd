...
1. Install EP Toolkit: https://sourceforge.net/projects/erppcatoolkit/
    * After downloading, copy the “EP_Toolkit folder” over to your “Matlab” folder
    * Open the “EP_Toolkit” folder
    * Open the “Documentation” folder
    * Open the “tutorial” document
    * Navigate to the “Set Up” section in the tutorial file and follow the instructions for installing and setting up the EP Toolkit, EEGLAB, and FieldTrip
    * You should now have the following subfolders in your MatLab folder:
        * EP_Toolkit
        * eeglab[version number]
        * Fieldtrip-[version number]
1. Install HAPPE pipeline: https://github.com/PINE-Lab/HAPPE
    * Clone HAPPE repository to GitHub 
    * Open the “HAPPE” folder
    * Open the “HAPPE User Guide” document
    * Read through the HAPPE User Guide
    * Navigate to the “Setting up HAPPE” section in the user guide and follow the instructions for setting up the HAPPE pipeline
    * Follow User Guide instructions for using the HAPPE-ER pipeline 

## Oddball Task
1. User Inputs
    * Enter the path to the folder containing the dataset. All ERP files are stored on the lab drive (Z:\Studies\School Readiness Study\Data\LV2\ERP)
    * Select "raw"
    * If this is your first time running data through the pipeline, indicate "N' for "Load pre-existing set of input parameters [Y/N]". If you have decided on a set of parameters. indicate "Y" and enter the path to the folder containing the input parameters. 
    * Low density data:N
    * Data type: task
    * Performing event-related potential (ERP) analysis: Y
    * Enter the task onset tags. When you have entered all tags, input "done". 
        * Target: tgt+
        * Frequent: frq+
    * Do multiple onset tags belong to a single condition? Indicate "N"
    * File format, indicate "5" for .mff(EGI) files
    * Acquisition layout type, indicate "2" for EGI Hydrocel Geodesic Sensor Net
    * Number of channels: 128
    * Do you have additioal type fields besides "code"? Indicate "N"
    * Select channels of interest, indicate "all" 
    * Frequency of electrical noice in Hz, indicate "60"
...
The Happe pipeline creates a file folder structure within the pipeline.
The pipeline exports the files in a number of desired file formats.
Ben and Alexis tried reading `.txt` files and `.set` files into the Toolkit.
However, `.txt` files had an extra channel, and `.set` files were not averaged accross trials.
